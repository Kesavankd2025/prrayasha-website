...
export default function GuestLoginPage() {
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true);
    try {
      const result = await apiAuth.sendGuestOtp(phoneNumber);
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const result = await apiAuth.verifyGuestOtp(phoneNumber, otpString);

      if (result.status) {
        const data = result.response;
        login(data.data.token, data.data.user);
        showToast(data.message || "Welcome Guest!", "success");
        router.push("/");
      } else {
        const errorData = result.response as any;
      ...
      <div className="auth-container">
        <div className="auth-card">
          <div className="auth-left">
            <img src="/logo.jpg" alt="Prrayasha Collections" className="auth-brand-logo" />
          </div>
              ...
              {otpSent ? (
                <div className="form-step animate-fade-in">
                  ...
                  <button className="auth-back-btn" onClick={() => setOtpSent(false)}>
                    <ArrowLeft size={16} /> Back to Guest Login
                  </button>
                </div>
              ) : (
                <div className="form-step animate-fade-in">
                  <h2 className="auth-title">Guest Login</h2>
                  ...
                  <div className="auth-helper-text">
                    <p>We will send you a 6 Digit Verification</p>
                    <p>Code</p>
                  </div>
                  ...
                  <div className="auth-footer-links">
                    ...
                    <Link href="/register" className="auth-link-btn underlined">Create an account</Link>
                  </div>
                </div>
              )}
            </div>
